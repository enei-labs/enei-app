...
import { useAuditIndustryBills } from "@utils/hooks/mutations";

interface IndustryBillBatchAuditModalProps {
  open: boolean;
  onClose: () => void;
  month: string;
  bills: IndustryBill[];
...
export const IndustryBillBatchAuditModal = ({
  open,
  onClose,
  month,
  bills,
  onSuccess,
}: IndustryBillBatchAuditModalProps) => {
  const [auditBills, { loading }] = useAuditIndustryBills();
  ...
  const handleConfirm = async () => {
    try {
      ...
      const updatedCount = data?.auditIndustryBills?.updatedCount ?? 0;
    ...
    <Dialog open={open} onClose={loading ? undefined : onClose} maxWidth="md" fullWidth>
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          ...
          <Typography variant="h6">批次審核 - 發電業電費單 {month}</Typography>
        </Box>
